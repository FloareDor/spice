...
def load_audio(
    ...
    import librosa
    # Use librosa for robustness on Windows
    try:
        # Load with original SR, no mono conversion yet (handled below)
        y, sample_rate = librosa.load(file_path, sr=None, mono=False)
        if y.ndim == 1:
